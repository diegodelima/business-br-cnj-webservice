...
This module is a straight brindge to the brazilian CNJ (Conselho Nacional de Justiça) SOAP WebServices.
...
Used internally only - all other methods use this to make the requests.

If an error happens, it return undef and sets the "errstr" attribute to the error value. Retrieve the error with the errstr method call.
